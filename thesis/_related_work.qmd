...
Pytifex builds on these foundations by combining MSR with modern language models. Rather than extracting syntactic patterns from bug-fixing commits, we mine closed issues from type checker
repositories (mypy, pyrefly, zuban, ty), extract minimal failing examples, and feed them to an LLM (Gemini) with instructions to generate semantically similar but distinct variations. This approach
addresses limitations in prior work: unlike SemSeed's pattern matching, LLM-based mutation can generalize bug characteristics to new contexts while maintaining semantic validity. Unlike purely
empirical studies, we use bug insights for proactive test generation rather than retrospective analysis. Recent work demonstrates the promise of this combination: Yang et al. developed WhiteFox, an
LLM-guided compiler fuzzing approach that outperforms template-based fuzzers by up to 8.2x in triggering optimizations, suggesting that LLM semantic understanding can exceed the effectiveness of
pattern-based approaches [@yang2024whitefox].
