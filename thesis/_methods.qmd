...
To exploit the second insight, we mine closed issues from type checker repositories (mypy, pyright, pyrefly, ty) and extract minimal reproducible examples. These seed bugs
are fed to a large language model (Gemini) with instructions to generate semantically similar variations that may trigger related failures in other checkers.
