...
Figure 1 \ref{fig:fig-false-mypy} illustrates such a case:
```{python}
import itertools
...
The code in Figuer 1 \ref{fig:fig-false-mypy} passes type checking in mypy without warnings, yet crashes at runtime with an AttributeError—the `itertools` module has no
