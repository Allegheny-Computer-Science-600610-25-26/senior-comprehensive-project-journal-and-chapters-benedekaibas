...
(FSE 2021) demonstrated that semantic bug seeding improved detection rates from 7% to 53% compared to artificial mutation. The curated GitHub
seeds provide the semantic guidance necessary to hit genuine type system edge cases.
...
definitively prove false negatives (missed bugs), but cannot prove false positives (spurious errors) since code may run successfully without
