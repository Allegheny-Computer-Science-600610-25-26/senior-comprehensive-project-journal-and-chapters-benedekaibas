...
positive derived from a reported issue. As shown in @fig-overload-error, distinct type checkers often disagree on the validity of complex
typing constructs.
...
demonstrates a critical safety gap involving Python's structural pattern matching (`match`/`case`). In this scenario, the developer attempts
to match against a non-existent attribute of the standard library module `itertools`.
